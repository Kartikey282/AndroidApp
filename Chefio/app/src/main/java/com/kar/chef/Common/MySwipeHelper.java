...
import com.kar.chef.Callback.MyButtonClickListener;
...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback
{
    ...
    public class MyButton {
        ...
        void onDraw (Canvas c, RectF rectF, int pos)
        {
            ...
            if (imageResId== 0)
            {
                ...
                y = cHeight/21 + r.height()/2f - r.bottom;
    ...
    private void drawButton(Canvas c, View itemView, List<MyButton> buffer, int pos, float translationX) {
        ...
        float dButtonWith = -1*translationX / buffer.size();
        for (MyButton button: buffer)
        {
            float left = right - dButtonWith;
